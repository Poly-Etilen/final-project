...
위한 읽기 전용 캐시입니다. Cultivation Service의 `cultivation_sensor` 테이블이
원본(source of truth)이며, 아래 두 가지 방식으로만 채워집니다.

- 평상시: Cultivation Service가 발행하는 `SensorRegisteredEvent`(Upsert)/`SensorDeletedEvent`(삭제) 구독
- 서비스 시작 시: Cultivation Service의 `GET /api/v1/cultivations/sensors`를 OpenFeign으로 호출해 전체 목록을 한 번에 채움 (재시작하면 메모리가 비기 때문)
...
SensorCacheEntry
──────────────────────────────────────────────
    deviceEui       (Key, Cultivation Service의 cultivation_sensor.device_eui와 동일)
    cultivationId
    sensorType
```

`location`/`locationDetail`/`deviceModel`/`status` 같은 상세 메타데이터는 갖지
않습니다. 필요하면 Cultivation Service API를 조회합니다.

---

# Status

`sensor_cache`는 상태(ONLINE/OFFLINE/ERROR 등)를 갖지 않습니다. 센서 상태는
Cultivation Service의 `cultivation_sensor.status`가 원본이며, Rule Engine Service가 발행하는
`SensorErrorEvent`도 Cultivation Service가 구독합니다. DatasourceGenerator는 상태 정보가
...
↓
Cultivation Service에 OpenFeign 호출 (GET /api/v1/cultivations/sensors)
↓
...
```
Cultivation Service (cultivation_sensor, 원본)
    │  SensorRegisteredEvent / SensorDeletedEvent (RabbitMQ)
    │  GET /api/v1/cultivations/sensors (OpenFeign, 서비스 시작 시에만)
    ▼
...
- 센서 "장치" 메타데이터의 원본(source of truth)은 Cultivation Service의
  `cultivation_sensor` 테이블입니다. DatasourceGenerator는 시뮬레이션/발행에 필요한
  최소 정보만 메모리에 보관합니다.
- 메모리 캐시이므로 서비스 재시작 시 데이터가 사라지지만, 시작 시점에 Cultivation
...
- 시계열 데이터는 Cultivation Service(InfluxDB)에서 관리합니다. Rule Engine Service는
  수신·검증·규칙평가만 담당하고 RabbitMQ로 Cultivation Service에 저장을 위임합니다.
