...
### 인사이트 (타인의 유사 재배 사례 후보 리스트 + 상세 조회)

```
수확 완료 (Cultivation Service → HarvestCompletedEvent)
↓
AI Service가 구독 → 환경 평균/생육 점수/한 줄 요약 조합 → insight 테이블 저장 (AI DB)
...
일일 피드백(자기 자신의 이력 비교)과 달리 타인의 사례를 보여주며, 스케줄 기반 push가
아닌 사용자 요청 시점(on-demand)에만 조회됩니다. 재배 기간이 사용자마다 달라 일자별/
단시간 비교로는 유의미한 차이가 나오지 않는다고 판단해, 재배 전체 기간 평균으로 유사
사례를 찾고 그 사례의 실제 기록을 그대로 보여주는 방식을 택했습니다. 사례 적재도
배치가 아니라 수확이 기록될 때마다 즉시 이루어지며, 검색 조건이 정확한 값/범위
필터라 별도의 벡터 검색 없이 인덱스가 걸린 SQL 조회로 충분합니다.
