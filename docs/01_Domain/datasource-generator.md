...
> 데이터를 만들지는 이벤트를 구독해서 판단합니다.

> ℹ️ **변경 이력**: 팀 회의 결과 `sensor` 테이블이 Cultivation Service에서 Sensor Service로
> 다시 이관되었습니다. 아래 이벤트 발행 주체와 재시작 시 캐시 재구성 호출 대상이 모두
> Cultivation Service에서 Sensor Service로 바뀌었습니다. (자세한 내용은
...
> 메모리가 비므로, 시작 시점에 Sensor Service를 OpenFeign으로 호출해(`GET
...
`sensor_cache`에 있는 센서(Sensor Service가 등록한 센서)에 대해서만 데이터를 생성하여
MQTT Broker로 발행합니다.
...
- **평상시(이벤트 기반)**: Sensor Service가 센서를 등록/삭제할 때 발행하는
  `SensorRegisteredEvent`/`SensorDeletedEvent`를 구독해 메모리 캐시를 갱신합니다.
  - 등록 시: sensor_cache에 Upsert
  - 삭제 시: sensor_cache에서 삭제
- **서비스 시작 시(재구성)**: 메모리 캐시는 재시작하면 비어 있으므로, Sensor Service의
  `GET /api/v1/sensors`를 OpenFeign으로 호출해 전체 센서 목록을 한 번에 받아와 캐시를 채웁니다.
  이후에는 다시 이벤트 기반으로 갱신됩니다.

센서 상태(ONLINE/OFFLINE/ERROR)는 Sensor Service의 책임입니다. Rule Engine
Service가 발행하는 SensorErrorEvent도 이 서비스가 구독하지 않습니다.
...
센서 "장치" 자체의 등록/조회/삭제는 [sensor-api.md](../02_API/sensor-api.md)를 참고하세요.
...
Sensor Service가 센서를 등록할 때 발행합니다. `sensor_cache`에 Upsert합니다.

---

### SensorDeletedEvent

Sensor Service가 센서를 삭제할 때 발행합니다. `sensor_cache`에서 해당 행을 삭제합니다.

---

발행하는 이벤트는 없습니다. 센서 데이터는 MQTT를 통해서만 전송합니다.

> ℹ️ **변경 이력**: SensorErrorEvent(Rule Engine Service 발행)는 이 서비스가 구독하지
> 않습니다. sensor.status의 원본이 DatasourceGenerator → Cultivation Service → Sensor
> Service 순으로 옮겨지면서 구독 주체도 그때마다 함께 이전했습니다. 이 서비스가 구독하는
> 이벤트는 위 SensorRegisteredEvent/SensorDeletedEvent(현재는 Sensor Service 발행)뿐입니다.
...
- 서비스 시작 시 Sensor Service 호출 실패 (캐시를 재구성하지 못해 재시도할 때까지 데이터 발행 불가)
