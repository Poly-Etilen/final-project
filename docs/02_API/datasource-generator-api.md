...
센서 "장치"의 등록/조회/삭제는 [cultivation-api.md](./cultivation-api.md)의 "센서 등록"/
"센서 목록 조회"/"센서 상세 조회"/"센서 삭제"를 참고하세요. 센서가 측정한 환경 값 자체의
조회는 [sensor-api.md](./sensor-api.md)를 참고하세요.

> ℹ️ **변경 이력**: 이전에는 이 서비스가 `/api/v1/sensors`로 센서 장치를 등록/조회하고,
> `/api/v1/datasources`로 데이터 소스(위치 그룹)를 관리했습니다. 센서 CRUD가 Cultivation
> Service(`/api/v1/cultivations/{id}/sensors`)로 옮겨지면서 `/api/v1/sensors` 경로 충돌
> 이슈가 해소되었고, 이어서 위치 정보(place/location)가 센서 레코드에 직접 저장되는 방식으로
> 바뀌면서 "데이터 소스" 개념 자체와 `/api/v1/datasources` API도 제거되었습니다. 그 결과 이
> 서비스에는 더 이상 REST API가 남아있지 않습니다.

> ℹ️ **변경 이력**: `sensor_cache`를 PostgreSQL이 아닌 메모리(In-Memory)에서만 관리하도록
> 바뀌면서, 서비스 시작 시 Cultivation Service의 `GET /api/v1/sensors`를 OpenFeign으로
> 호출해 전체 센서 목록을 받아와 캐시를 재구성합니다. (자세한 내용은
> [cultivation-api.md](./cultivation-api.md)의 "센서 전체 목록 조회 (내부용)" 참고)
...
| 상황 | 설명 |
|------|------|
| sensor_cache에 없는 센서 | Cultivation Service 이벤트가 아직 반영되지 않았거나 유실됨 |
...
| 서비스 시작 시 Cultivation Service 호출 실패 | 메모리 캐시를 재구성하지 못해 재시도할 때까지 데이터 발행 불가 |
...
```
Cultivation Service - GET /api/v1/sensors (서비스 시작 시에만, 메모리 캐시 재구성용)
```
