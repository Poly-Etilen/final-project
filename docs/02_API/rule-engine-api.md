...
센서 "장치" 등록/조회/삭제와 환경 설정(목표 환경) 저장/조회, 측정값 현재값/통계/차트/리포트
조회는 모두 [sensor-api.md](./sensor-api.md)를 참고하세요.
...
> ℹ️ **변경 이력**: 센서 "장치" CRUD API는 원래 [cultivation-api.md](./cultivation-api.md)에
> 있었으나, `sensor`/`environment_setting` 테이블과 함께 Sensor Service로 완전히
> 이관되었습니다. 아래 OpenFeign/RabbitMQ 섹션의 호출·발행 대상도 Cultivation Service에서
> Sensor Service로 함께 바뀌었습니다.
...
SensorErrorEvent → Sensor Service (sensor.status 갱신), Notification Service (센서 오류 감지 시 발행)
```

Subscribe

```
EnvironmentRangeUpdatedEvent (Sensor Service 발행) → Redis 캐시(cultivation:{cultivationId}:range) 갱신
```
...
```
Sensor Service (Redis 캐시 미스 시에만 호출하는 fallback — 목표 환경 범위(min~max) 조회)
```
...
| Redis 캐시 조회/저장 실패 | 목표 환경 범위 캐시 사용 불가, Sensor Service fallback 호출 증가 |
