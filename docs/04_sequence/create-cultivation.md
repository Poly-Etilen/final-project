...
└── cultivation_member 생성 (생성자를 role = OWNER로 자동 등록)
↓
mushroom_reference + mushroom_reference_threshold 조회 (같은 DB, 내부 조회)
↓ (디바이스가 있다면)
cultivation_sensor 배치 등록 (같은 트랜잭션, 실패 시 롤백)
↓
환경 추천 + registeredSensors 반환
↓
Client
↓ (선택)
AI Service — 버섯 가이드 조회 (효능/주의사항, mushroomId 기준)
↓
...
```json
{
    ...
    "mushroomId": 3,
    "devices": [
        { "deviceEui": "24e124128c067999", "location": "A동", "locationDetail": "선반1", "deviceModel": "Milesight AM107", "sensorTypeIds": [1, 2, 3, 4] }
    ]
}
```

`sensorTypeIds`는 `sensor_type.id`를 가리킵니다(시드 데이터: `1`=온도, `2`=습도,
`3`=CO2, `4`=조도 — `GET /sensor-types`로 목록/이름을 조회할 수 있습니다). 기존에는
`measurement_type`이 자연키 코드(`TEMPERATURE` 등)라 코드 문자열을 그대로 주고받았지만,
`sensor_type`이 대리키로 바뀌면서 id로 주고받습니다.
...
정보는 저장하지 않음). 같은 트랜잭션 안에서 요청자를 `role = OWNER`로 하는
`cultivation_member` 행도 함께 생성합니다 — 이후 이 재배에 다른 사용자를 멤버로
초대할 수 있는 기반이 됩니다. `devices`가 1개 이상이면 역시 같은 트랜잭션 안에서
`device_eui`별로 `cultivation_sensor` 행을 생성하고(PK는 대리키 `id`, `device_eui`는
UNIQUE 제약을 가진 일반 컬럼), `sensorTypeIds`별로 `cultivation_sensor_type` 행도
함께 생성합니다.
...
직접 조회합니다(내부 조회, `GET /api/v1/mushroom-references/{mushroomId}`도 다른
서비스를 위해 내부용으로 계속 제공). `mushroom_reference_threshold`는 `sensor_type_id`로
항목을 구분하므로, 조회 시 `sensor_type`과 JOIN해 `sensorType`(예: 온도) 이름과
`valueUnit`(예: ℃)을 함께 가져옵니다(단위를 `mushroom_reference_threshold`에 직접
저장하지 않음).
...
```json
{
    ...
    "thresholds": [
        { "sensorTypeId": 1, "sensorType": "온도", "min": 15.0, "max": 18.0, "unit": "℃" },
        { "sensorTypeId": 2, "sensorType": "습도", "min": 85, "max": 95, "unit": "%" },
        { "sensorTypeId": 3, "sensorType": "CO2", "min": 700, "max": 900, "unit": "ppm" },
        { "sensorTypeId": 4, "sensorType": "조도", "min": 300, "max": 400, "unit": "lux" }
    ]
}
```

`unit`은 `mushroom_reference_threshold`에 저장된 값이 아니라, `sensor_type_id`로
`sensor_type.value_unit`을 JOIN해 채운 값입니다.
...
Service를 거치지 않으며, `cultivationId`도 필요하지 않습니다(`mushroomId`에만 의존).
...
문장으로 재구성하고, `mushroomId` 기준으로 캐싱합니다(TTL 7일). 버섯 종류가 5종
...
```
Temperature 22℃ → sensorTypeId=1(온도), min 20.5 / max 23.5
Humidity 91% → sensorTypeId=2(습도), min 86 / max 96
```
...
```
cultivation
cultivation_member (생성자를 OWNER로 자동 등록)
cultivation_sensor / cultivation_sensor_type (devices를 함께 등록한 경우)
mushroom_reference / mushroom_reference_threshold (조회 전용, sensor_type과 JOIN)
environment_setting (sensor_type_id FK)
```
...
- 존재하지 않는 `mushroomId` (`mushroom_reference`에 없음)
...
- `environment_setting`/`mushroom_reference_threshold` 모두 `threshold_unit` 컬럼이
  없으므로, 단위가 필요한 조회(추천 환경, 환경 설정 현재값 등)는 항상 `sensor_type`과
  JOIN해 `value_unit`을 함께 가져와야 합니다.
