...
1. **인사이트 사례 적재** (이벤트 기반, push): Cultivation Service가 수확(harvest)을
   ...
   환경 평균(Sensor Service 조회)과 최근 생육 점수(`growth_record` 자체 조회)를 묶어
...
AI Service (구독)
↓
Sensor Service OpenFeign 호출 (GET /../environment-average, 해당 재배의 환경 평균 조회)
↓
...
+ Sensor Service OpenFeign 호출 (GET /../environment-average, 환경 평균 조회)
↓
...
이벤트에 담긴 `cultivationId`로 Sensor Service에 환경 평균을 조회합니다.

```http
GET /api/v1/sensors/cultivations/12/environment-average
```
...
`avgTemperature`~`avgLight`는 Sensor Service가 `environment_setting` 이력 전체를
...
버섯 종류는 Cultivation Service에서, 환경 평균은 Sensor Service에서 각각 조회합니다.

```http
GET /api/v1/cultivations/27
GET /api/v1/sensors/cultivations/27/environment-average
```
...
- 인사이트 적재 시 Sensor Service 호출 실패 (환경 평균 조회 실패 — 해당 수확의
  insight 적재는 건너뛰며, 이벤트 유실/실패에 대비한 재처리는 추후 개발 예정)
- `insight` 테이블 저장 실패
- 인사이트 조회 시 Cultivation/Sensor Service 호출 실패
...
- 인사이트 사례 적재는 `HarvestCompletedEvent` 구독으로 트리거되며, 인사이트 조회만
  사용자가 직접 요청합니다.
- 환경 평균은 "기간 가중 평균"입니다. Sensor Service가 `environment_setting` 이력을
